...
bool DetectionTemporal::runDetection(Frame &c) {
    ...
    if(!mSubdivisionStatus) {
        ...
        if(!mMaskManager->applyMask(currImg)) {
            ...
            // Liste d'iterators sur la liste des localEvent contenant soit un cluster positif ou negatif.
            vector<vector<LocalEvent>::iterator > itLePos, itLeNeg;

            // Association d'un local event cluster positif avec un local event  cluster negatif.
			...
			//cerr<<"Meteor "<<dur_elapsed.count()<<endl;
            return saveSignal;
